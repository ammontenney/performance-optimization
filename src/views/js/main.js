...
var resizePizzas = function(size) {
  ...
  // function determineDx was removed because it was unnecessarily complicated


  // Iterates through pizza elements on the page and changes their widths
  function changePizzaSizes(size) {
    // we only need to query the DOM once for the entire loop
    // getElementsByClassName is faster than querySelectorAll
    var pizzas = document.getElementsByClassName("randomPizzaContainer");

    // this snippet below eliminates FSL by removing calls to layout as well as
    // eliminates the unnecessarily complicated determineDx function
    var newwidth = 0;
    if (size == '1') newwidth = '25%';
    else if (size == '2') newwidth = '33%';
    else if (size == '3') newwidth = '50%';
...
function updatePositions() {
    ...
    // This is a small optimization. We only need to run querySelectorAll
    // once instead of every time we run this function
    if (!items) items = document.getElementsByClassName('mover');

    var scrlTop = lastKnownScrollTop;

    // Another small optimization. Precalculate the phases for pizza movement.
    var phases =   [Math.sin((scrlTop / 1250) + 0),
                    Math.sin((scrlTop / 1250) + 1),
                    Math.sin((scrlTop / 1250) + 2),
...
function onScroll(){
    // Debounce scroll events. This also eliminates FSL
    lastKnownScrollTop = document.body.scrollTop;
    requestTick();
}

function requestTick(){
    // once the last rAF finishes, request another while we are still scrolling
    if (!scrollTicking) requestAnimationFrame(updatePositions);
    scrollTicking = true;
}
